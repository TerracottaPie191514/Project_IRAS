...
wilcox.test(lib.div$diversity_shannon ~ sample_data(subsetMG)$AB) # shannon diversity does not seem to significantly differ across the different AB groups

wilcox.test(lib.div$diversity_gini_simpson ~ sample_data(subsetMG)$AB) # not significant

wilcox.test(lib.div$evenness_pielou ~ sample_data(subsetMG)$AB) # not significant
...
TukeyHSD(aov.invsimp.farm) # only not significant between 1 and 4 and 3 and 2
...
pairwise.wilcox.test(lib.div$diversity_shannon, sample_data(subsetMG)$Farm2, p.adjust.method="fdr") # difference between 1 and 3 and 4 and all other farms

kruskal.test(lib.div$diversity_gini_simpson ~ sample_data(subsetMG)$Farm2) # significant
pairwise.wilcox.test(lib.div$diversity_gini_simpson, sample_data(subsetMG)$Farm2, p.adjust.method="fdr") # difference between farm 4 and 2 and 4 and 3

kruskal.test(lib.div$evenness_pielou ~ sample_data(subsetMG)$Farm2) # significant
pairwise.wilcox.test(lib.div$evenness_pielou, sample_data(subsetMG)$Farm2, p.adjust.method="fdr") # difference between farm 4 and the other farms
...
pairwise.wilcox.test(lib.div$diversity_gini_simpson, sample_data(subsetMG)$Cox, p.adjust.method="fdr") # only diff between sacox and maxiban and sacox and none
...
kruskal.test(lib.div$evenness_pielou ~ sample_data(subsetMG)$Cox) # significant
pairwise.wilcox.test(lib.div$evenness_pielou, sample_data(subsetMG)$Cox, p.adjust.method="fdr") # same as above
...
aov.shannon.age_farm = aov(lib.div$diversity_shannon ~ sample_data(subsetMG)$Age*sample_data(subsetMG)$Farm2)
summary(aov.shannon.age_farm)

aov.shannon.age_farm = aov(lib.div$diversity_shannon ~ sample_data(subsetMG)$Age+sample_data(subsetMG)$Farm2)
summary(aov.shannon.age_farm)

aov.shannon.age_AB = aov(lib.div$diversity_shannon ~ sample_data(subsetMG)$Age*sample_data(subsetMG)$AB)
...
aov.shannon.age_all = aov(lib.div$diversity_shannon ~ sample_data(subsetMG)$Age*sample_data(subsetMG)$AB*sample_data(subsetMG)$Farm2*sample_data(subsetMG)$Cox*sample_data(subsetMG)$FarmRoundStable)
summary(aov.shannon.age_all)

# repeated measures, look at second guide to figure this out

rm.shannon.all = lmer(lib.div$diversity_shannon ~ sample_data(subsetMG)$AB + (1|sample_data(subsetMG)$Farm2))
